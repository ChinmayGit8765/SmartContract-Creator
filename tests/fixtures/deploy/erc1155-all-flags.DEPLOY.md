...
### Owner-controlled URI (**CRITICAL**)

ERC-1155 default-URI setter is owner-controlled (wizard default `updatableUri:true`). The contract owner can change the URI template at any time. Use a multisig owner or freeze ownership before launch if metadata must be immutable.
...
2. In the File Explorer, create a new file named `MyMulti.sol`
3. Paste the contents of your generated `MyMulti.sol` into it
4. In the **Solidity Compiler** tab, select compiler 0.8.35 and click "Compile MyMulti.sol"
5. In the **Deploy & Run** tab, choose your environment (Injected Provider — MetaMask), enter the constructor arguments (defaultAdmin, pauser, minter), and click **Deploy**
...
async function main() {
  const MyMulti = await hre.ethers.getContractFactory("MyMulti");
  const contract = await MyMulti.deploy(
...
forge create src/MyMulti.sol:MyMulti \
...
forge verify-contract <DEPLOYED_ADDRESS> src/MyMulti.sol:MyMulti \
  --constructor-args $(cast abi-encode "constructor(address,address,address)" <YOUR_WALLET_ADDRESS> <YOUR_WALLET_ADDRESS> <YOUR_WALLET_ADDRESS>) \
...
npx hardhat verify --network <NETWORK_NAME> <DEPLOYED_ADDRESS> "<YOUR_WALLET_ADDRESS>" "<YOUR_WALLET_ADDRESS>" "<YOUR_WALLET_ADDRESS>"
```
...
| - | ---- | ---- | ------------- |
| 1 | `defaultAdmin` | `address` | `<YOUR_WALLET_ADDRESS>` |
| 2 | `pauser` | `address` | `<YOUR_WALLET_ADDRESS>` |
| 3 | `minter` | `address` | `<YOUR_WALLET_ADDRESS>` |
